...
def simplicity_pool(simp,pool):
        ...
        for i, (l, div) in enumerate(zip(layers,divs)):
                try:
                        pool.apply_async(run_ratio,(j, div, l, "background", 0,i+shift_gpu,i))
                        #def run_ratio(simp_level, div, layer_opt, object_or_background, resize_obj,gpu_id, process_id):
        ...
        if args.fg_bg_separation:
                for i, (l,div) in enumerate(zip(layers,divs)):
                        try:
                                pool.apply_async(run_ratio,(j, div, l, "object",1, i+shift_gpu,i+3))
                                #def run_ratio(simp_level, div, layer_opt, object_or_background, resize_obj,gpu_id, process_id):
                        except Exception as e:
                                print(f"An error occurred: {e}")
                # P4.close()
        ...
        for i, (l, div) in enumerate(zip(layers,divs)):
                try:
                        pool.apply_async(run_ratio,(j+1, div, l, "background", 0,i+3+shift_gpu,i+6))
                        #def run_ratio(simp_level, div, layer_opt, object_or_background, resize_obj,gpu_id, process_id):
        ...
        if args.fg_bg_separation:
                for i, (l,div) in enumerate(zip(layers,divs)):
                        try:
                                pool.apply_async(run_ratio,(j+1, div, l, "object",1, i+3+shift_gpu,i+9))
                                #def run_ratio(simp_level, div, layer_opt, object_or_background, resize_obj,gpu_id, process_id):
                        except Exception as e:
                                print(f"An error occurred: {e}")
                # P4.close()
...
if __name__ == "__main__":
        ...
        # P2.apply_async(run_ratio,(1, divs[2], layers[2], "object",1, 1+3+shift_gpu,2+9))
        # P2.apply_async(run_ratio,(1, divs[2], layers[2], "background",1, 1+3+shift_gpu,2+9))
        P2.close()
